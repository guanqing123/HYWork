...
+ (void)getHomeNotices:(WKHomeNoticeParam *)noticeParam success:(void (^)(WKHomeNoticeResult * _Nonnull))success failure:(void (^)(NSError * _Nonnull))failure {
    
    NSString *urlStr = @"http://dev.sge.cn/rest/hywork/home/notices/list";
    
    NSDictionary *parameter = [noticeParam mj_keyValues];
    
    [WKHttpTool postWithUrl:urlStr param:parameter success:^(id responseObject) {
        WKHomeNoticeResult *result = [WKHomeNoticeResult mj_objectWithKeyValues:responseObject];
        success(result);
    } failure:^(NSError *error) {
        failure(error);
    }];
