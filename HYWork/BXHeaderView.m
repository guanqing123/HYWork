...
+ (instancetype)headerViewWithTableView:(UITableView *)tableView {
    ...
    BXHeaderView *header = [tableView dequeueReusableHeaderFooterViewWithIdentifier:ID];
    if (header == nil) {
        header = [[BXHeaderView alloc] initWithReuseIdentifier:ID];
    }
    return header;
}
...
/**
 * 在这个初始化方法中,BXHeaderView的frame\bounds没有值
 */
...
- (void)setGroup:(BXGroup *)group {
    _group = group;
    
    // 1.设置按钮文字(组名)
    [self.nameView setTitle:group.lb forState:UIControlStateNormal];
    
    // 2.设置报销单数
    self.countView.text = [NSString stringWithFormat:@"[%@]",group.size];
